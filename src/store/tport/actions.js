...
export const updateTeleports = async function ({ commit }, account) {
  try {
    if (account !== null) {
      ...
      // Get cancelled teleport ids and filter them out
      let cancelledResp = await this.$api.getTableRows({
        code: process.env.TPORT_ADDRESS,
        scope: process.env.TPORT_ADDRESS,
        table: "cancels",
        reverse: true,
      ...
      let cancelledIds = [];
      cancelledResp.rows.forEach((r) => {
        cancelledIds.push(r.teleport_id);
      });
